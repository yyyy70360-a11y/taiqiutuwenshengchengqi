...
- 16 套 SVG 模板输出固定 `1080 x 1920` PNG。
...
- 16 套模板样例图生成通过；100 张离线渲染压力测试单独执行通过，耗时 `263.26s`。
- 服务端测试：`14 passed`，覆盖认证、后台 HTML 转义、AI 配置校验、AI 文案解析和模板容量兜底。
...
- Mac 前端已将当前模板传给 AI 网关；服务端会按 16 套模板容量规则再次收口。
...
- 首批 6 套模板改用明确的水平布局网格；卡片型模板共用 `60,220,960,1480` 外框，标题、正文、分隔线和话题按模板内边距对齐，长文本与话题不会越过边框。
